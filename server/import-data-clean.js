...
try {
  ...
  // 2026-10以降のデータを削除
  db.exec("DELETE FROM orders WHERE deliveryMonth >= '2026-10'");
  ...
  data.forEach(item => {
    Object.entries(item.months).forEach(([month, amount]) => {
      const id = `order-2026-${month}-${item.customer}-${item.name}`;
  ...
  console.log(`✓ ${count}件のレコードをインポートしました`);
  ...
  months.forEach(month => {
    ...
    const monthTotal = (result.total || 0) / 10000;
    total += monthTotal;
