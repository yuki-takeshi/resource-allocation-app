...
function CustomerMonthlyDashboard({ user }) {
  ...
  useEffect(() => {
    ...
    if (customerOrder.length === 0 && uniqueCustomers.length > 0) {
      setCustomerOrder(uniqueCustomers);
    }
  }, [getFilteredOrders().map(o => o.customer).join(',')]);
  ...
  const getDisplayCustomers = () => {
    ...
    if (customerOrder.length > 0) {
      return customerOrder.filter(c => regularCustomers.includes(c))
        .concat(regularCustomers.filter(c => !customerOrder.includes(c)));
    }
  ...
  const handleDrop = (e, targetCustomer) => {
    ...
    try {
      ...
      const newOrder = [...displayCustomers];
  ...
  const customers = getDisplayCustomers();
