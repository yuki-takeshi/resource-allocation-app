...
const DEPARTMENTS = ['All', '開発事業部', '営業部', 'MGその他'];
const RANK_COLORS = {
...
function formatCurrency(value, deliveryMonth = null) {
  if (!value) return '0万円';

  // deliveryMonthが2026-10以降の場合は万円単位、それ以前は円単位
  if (deliveryMonth && (deliveryMonth >= '2026-10')) {
    // 既に万円単位で保存されている
    return `${Math.floor(value)}万円`;
  } else {
    // 円単位で保存されているので万円に変換
...
function formatCurrencyFull(value, deliveryMonth = null) {
  if (!value) return '0万円';

  // deliveryMonthが2026-10以降の場合は万円単位、それ以前は円単位
  if (deliveryMonth && (deliveryMonth >= '2026-10')) {
    // 既に万円単位で保存されている
    return `${Math.floor(value).toLocaleString('ja-JP')}万円`;
  } else {
    // 円単位で保存されているので万円に変換
...
function PrintDialog({ isOpen, selectedSections, onToggleSection, onPrint, onCancel }) {
  if (!isOpen) return null;

  const sections = [
    { id: 'stuckOrders', label: '滞留案件一覧', checked: selectedSections.stuckOrders },
    { id: 'expectationVsTarget', label: '見込みvs目標', checked: selectedSections.expectationVsTarget },
    { id: 'analysis', label: 'ランク別分析', checked: selectedSections.analysis },
    { id: 'filters', label: 'フィルター（部署・検索）', checked: selectedSections.filters },
    { id: 'trend', label: '受注残推移', checked: selectedSections.trend },
    { id: 'rankDistribution', label: 'ランク分布・月別実績VS目標', checked: selectedSections.rankDistribution },
    { id: 'monthlyTargets', label: '月別目標設定', checked: selectedSections.monthlyTargets },
    { id: 'completedOrders', label: '完了案件一覧', checked: selectedSections.completedOrders },
    { id: 'lossOrders', label: '失注案件一覧', checked: selectedSections.lossOrders },
    { id: 'changeLog', label: '変更履歴', checked: selectedSections.changeLog }
  ];

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-8 max-w-2xl w-full max-h-96 overflow-y-auto">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">印刷するセクションを選択</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          {sections.map(section => (
            <label key={section.id} className="flex items-center gap-3 cursor-pointer p-3 hover:bg-gray-50 rounded">
              <input
                type="checkbox"
...
function OrderForm({ order, onSave, onCancel }) {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'amount') {
      // 一旦入力値のみ保存（単位変換は handleSubmit で行う）
      setFormData(prev => ({
        ...prev,
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.deliveryMonth >= '2026-10') {
      // 2026-10～2027-09 は万円で入力、そのまま保存
      finalAmount = Math.round(formData.amount * 100) / 100;
    } else {
      // 2025-10～2026-09 は万円で入力、円で保存（× 10000）
      finalAmount = Math.round(formData.amount * 10000);
    }

    // registerMode と periodEndMonth をフォームデータに含める
    // 新規登録の場合、targetAmount は設定しない（0のまま）
    // 既存データの場合のみ targetAmount を保持
    const dataToSave = {
      ...formData,
      amount: finalAmount,
      registerMode,
    ...
    // order が存在する場合（編集）のみ targetAmount を含める
    if (order && order.targetAmount !== undefined) {
      dataToSave.targetAmount = order.targetAmount;
    }
...
function OrderBacklogApp() {
  ...
  // 期間指定（納期月）のlocalStorage保存
  const [filterPeriodStart, setFilterPeriodStart] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('filterPeriodStart') || '';
    }
    return '';
  });
  const [filterPeriodEnd, setFilterPeriodEnd] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('filterPeriodEnd') || '';
    }
    return '';
  });
  ...
  const [selectedForecastPeriod, setSelectedForecastPeriod] = useState(() => {
    const saved = localStorage.getItem('selectedForecastPeriod');
    return saved || '2026-2026';
  });

  // 期間に応じて periodStart/periodEnd を計算
  const [periodStart, setPeriodStart] = useState(() => {
    const saved = localStorage.getItem('selectedForecastPeriod');
    const period = saved || '2026-2026';
    return period === '2026-2026' ? '2026-01' : '2026-10';
  });

  const [periodEnd, setPeriodEnd] = useState(() => {
    const saved = localStorage.getItem('selectedForecastPeriod');
    const period = saved || '2026-2026';
    return period === '2026-2026' ? '2026-12' : '2027-09';
  });

  useEffect(() => {
    if (selectedForecastPeriod === '2026-2026') {
      setPeriodStart('2026-01');
      setPeriodEnd('2026-12');
    } else if (selectedForecastPeriod === '2026-2027') {
      setPeriodStart('2026-10');
      setPeriodEnd('2027-09');
    }
    localStorage.setItem('selectedForecastPeriod', selectedForecastPeriod);
      ...
      try {
        ...
        if (!existingSnapshot) {
          const activeOrders = orders.filter(o =>
            o.status === '受注確定' || (o.status === '進行中' && (o.rank === 'A' || o.rank === 'B'))
          );
  ...
  const handleSaveTarget = async () => {
    ...
    try {
      const targetAmount = (parseInt(newTargetAmount) || 0) * 10000; // 万円から円に変換
  ...
  // スナップショットフィルター（実績設定期間に連動）
  const getFilteredSnapshots = () => {
    if (snapshots.length === 0) return [];

    // periodStart, periodEndは「2026-01」のような形式
    // 開始日：periodStart の開始日（例：2026-01-01）
    // 終了日：periodEnd の終了日（例：2026-12-31）
    const startDateStr = periodStart ? `${periodStart}-01` : null;
    const endYear = periodEnd ? parseInt(periodEnd.split('-')[0]) : null;
    const endMonth = periodEnd ? parseInt(periodEnd.split('-')[1]) : null;

    if (!startDateStr || !endYear || !endMonth) {
      return snapshots.sort((a, b) => a.date.localeCompare(b.date));
    }

    // 終了月の最終日を計算
    const endDate = new Date(endYear, endMonth, 0); // 月の最終日
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="max-w-7xl mx-auto p-8">
        ...
        <div className="print-header mb-8">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex items-center gap-2 bg-amber-600 hover:bg-amber-700 text-white font-medium py-3 px-6 rounded-lg shadow transition"
              >
                🖨️ 印刷
              </button>
              <button
          ...
          {/* 実績設定期間 */}
          <div className="filter-section mb-4 bg-gradient-to-r from-purple-50 to-blue-50 p-4 rounded-lg border-2 border-purple-200">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (typeof window !== 'undefined') {
                      localStorage.setItem('filterPeriodStart', val);
                    }
                  }}
                  className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (typeof window !== 'undefined') {
                      localStorage.setItem('filterPeriodEnd', val);
                    }
                  }}
                  className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                />
          ...
          {/* フィルター：ランク選択 */}
          <div className="filter-section bg-white p-4 rounded-lg border border-gray-200">
        ...
        <div className="mb-6 flex gap-4">
          <button
            onClick={() => setSelectedForecastPeriod('2026-2026')}
            className={`px-6 py-2 rounded-lg font-semibold transition ${
              selectedForecastPeriod === '2026-2026'
          ...
          <button
            onClick={() => setSelectedForecastPeriod('2026-2027')}
            className={`px-6 py-2 rounded-lg font-semibold transition ${
              selectedForecastPeriod === '2026-2027'
                ? 'bg-purple-600 text-white'
                : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
          >
        ...
        <>
        <div className="expectation-vs-target-section bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg shadow-lg p-8 mb-8 border-2 border-purple-200">
          ...
          <div className="grid grid-cols-2 gap-6">
                ...
                <div className="bg-blue-50 rounded-lg shadow p-6 border-l-4 border-blue-500">
                  ...
                  <div className="space-y-2 text-sm">
                    <p className="flex justify-between">
                      ...
                      <span className="font-bold text-black">{formatCurrencyFull(25590.8 * 10000)}</span>
                    </p>
                    <div>
                    ...
                    <p className="flex justify-between border-t pt-2">
                      ...
                      <span className={`font-bold ${totalActual - (25590.8 * 10000) >= 0 ? 'text-blue-900' : 'text-red-600'}`}>
                        {totalActual - (25590.8 * 10000) >= 0 ? '+' : ''}{formatCurrencyFull(totalActual - (25590.8 * 10000))}
                      </span>
                    </p>
                  </div>
            ...
            {(() => {
              ...
              let formerEnd = periodEnd;
              if (periodMonths > 6) {
                const midMonth = startMonth + Math.floor(periodMonths / 2) - 1;
                if (midMonth > 12) {
                  formerEnd = `${startYear + 1}-${String(midMonth - 12).padStart(2, '0')}`;
                } else if (midMonth > 0) {
                  formerEnd = `${startYear}-${String(midMonth).padStart(2, '0')}`;
                } else {
                  formerEnd = `${startYear - 1}-12`;
                }
              }

              const formerStats = getPeriodStats(periodStart, formerEnd);
              const completedActual = completedOrdersInPeriod.filter(order => order.deliveryMonth >= periodStart && order.deliveryMonth <= formerEnd).reduce((sum, o) => sum + o.amount, 0);
              const forecastActual = filteredOrders.filter(order => order.deliveryMonth >= periodStart && order.deliveryMonth <= formerEnd && (filterPeriodStart ? order.deliveryMonth >= filterPeriodStart : true) && (filterPeriodEnd ? order.deliveryMonth <= filterPeriodEnd : true)).reduce((sum, o) => sum + o.amount, 0);
              const totalActual = completedActual + forecastActual;
              const forecastA = filteredOrders.filter(o => o.rank === 'A' && o.deliveryMonth >= periodStart && o.deliveryMonth <= formerEnd && (filterPeriodStart ? o.deliveryMonth >= filterPeriodStart : true) && (filterPeriodEnd ? o.deliveryMonth <= filterPeriodEnd : true)).reduce((sum, o) => sum + o.amount, 0);
              const forecastB = filteredOrders.filter(o => o.rank === 'B' && o.deliveryMonth >= periodStart && o.deliveryMonth <= formerEnd && (filterPeriodStart ? o.deliveryMonth >= filterPeriodStart : true) && (filterPeriodEnd ? o.deliveryMonth <= filterPeriodEnd : true)).reduce((sum, o) => sum + o.amount, 0);
              const forecastC = filteredOrders.filter(o => o.rank === 'C' && o.deliveryMonth >= periodStart && o.deliveryMonth <= formerEnd && (filterPeriodStart ? o.deliveryMonth >= filterPeriodStart : true) && (filterPeriodEnd ? o.deliveryMonth <= filterPeriodEnd : true)).reduce((sum, o) => sum + o.amount, 0);
              const forecastD = filteredOrders.filter(o => o.rank === 'D' && o.deliveryMonth >= periodStart && o.deliveryMonth <= formerEnd && (filterPeriodStart ? o.deliveryMonth >= filterPeriodStart : true) && (filterPeriodEnd ? o.deliveryMonth <= filterPeriodEnd : true)).reduce((sum, o) => sum + o.amount, 0);

              return (
                <div className="bg-blue-50 rounded-lg shadow p-6 border-l-4 border-blue-500">
                  <p className="text-sm font-medium text-gray-700 mb-3">上期（{periodStart}～{formerEnd}）</p>
                  <div className="space-y-2 text-sm">
                    <p className="flex justify-between">
                      <span className="text-gray-600">目標:</span>
                      <span className="font-bold text-orange-600">{formatCurrencyFull(12647.1 * 10000)}</span>
                    </p>
                    <div>
                      <p className="flex justify-between mb-2">
                        <span className="text-gray-600">見込み:</span>
                    ...
                    <p className="flex justify-between border-t pt-2">
                      <span className="text-gray-600">差分:</span>
                      <span className={`font-bold ${totalActual - (12647.1 * 10000) >= 0 ? 'text-blue-900' : 'text-red-600'}`}>
                        {totalActual - (12647.1 * 10000) >= 0 ? '+' : ''}{formatCurrencyFull(totalActual - (12647.1 * 10000))}
                      </span>
                    </p>
                  </div>
                  <div className="mt-4 pt-4 border-t">
                ...
                <tbody>
                  {(() => {
                    ...
                    return groupNames.flatMap((groupName, groupIndex) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-3 px-4 text-right font-medium text-gray-700">
                              {formatCurrencyFull(order.amount, order.deliveryMonth)}
                            </td>
                      ...
                      const groupDeliveryMonth = groupItems[0].deliveryMonth; // グループ内の最初の案件の納期月を使用
                      ...
                      const rows = [
                        ...
                        >
                          ...
                          <td className="py-4 px-4 text-right font-bold text-blue-700">
                            {formatCurrencyFull(totalAmount, groupDeliveryMonth)}
                          </td>
                          <td colSpan="2"></td>
                        </tr>
                      ];
        ...
        {/* ランク別分析 */}
        <div className="analysis-section bg-white rounded-lg shadow-lg p-6 mb-8">
        ...
        {stuckOrders.length > 0 && (
          <div className="stuck-orders-section bg-gray-50 rounded-lg shadow-lg p-6 mb-8 border-l-4 border-red-500">
        ...
        {/* 受注残推移グラフ */}
        <div className="trend-section bg-white rounded-lg shadow-lg p-6 mb-8">
          <div className="mb-6">
            <h2 className="text-lg font-bold text-gray-900">受注残推移</h2>
            <p className="text-sm text-gray-600 mt-1">上の「期間指定（納期月）」に連動します</p>
          </div>
          {filteredSnapshots.length > 0 ? (
            <ResponsiveContainer width="100%" height={400}>
              <LineChart data={filteredSnapshots}>
                ...
                />
                <Tooltip
                  formatter={(value) => `${Math.floor(value / 10000)}M`}
        ...
        {/* グラフエリア */}
        <div className="rank-distribution-section grid grid-cols-2 gap-8 mb-8">
          {/* ランク分布 */}
        ...
        {/* 目標設定テーブル */}
        <div className="monthly-targets-section bg-white rounded-lg shadow-lg p-6 mb-8">
          <div className="flex justify-between items-center mb-6">
        ...
        {(() => {
          ...
          return completedOrders.length > 0 ? (
            <div className="completed-orders-section bg-white rounded-lg shadow-lg p-6 mb-8 border-l-4 border-blue-900">
        ...
        {(() => {
          ...
          return lossOrders.length > 0 ? (
            <div className="loss-orders-section bg-gray-50 rounded-lg shadow-lg p-6 mb-8 border-l-4 border-red-500">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <AlertCircle className="text-red-600" size={24} />
        ...
        {/* 変更履歴 */}
        <div className="change-log-section bg-white rounded-lg shadow-lg p-6">
          <div className="flex justify-between items-center mb-4">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">目標金額（万円） *</label>
                <input
                  type="number"
      ...
      <PrintDialog
        isOpen={printMode}
        selectedSections={selectedSectionsForPrint}
      ...
      <style>{`
        ...
        @media print {
          ...
          button, .flex.justify-between.items-center > div:last-child {
            display: none !important;
          }
          ${!selectedSectionsForPrint.filters ? '.filter-section { display: none !important; }' : ''}
          ${!selectedSectionsForPrint.trend ? '.trend-section { display: none !important; }' : ''}
          ${!selectedSectionsForPrint.rankDistribution ? '.rank-distribution-section { display: none !important; }' : ''}
          ${!selectedSectionsForPrint.monthlyTargets ? '.monthly-targets-section { display: none !important; }' : ''}
          ${!selectedSectionsForPrint.completedOrders ? '.completed-orders-section { display: none !important; }' : ''}
          ${!selectedSectionsForPrint.lossOrders ? '.loss-orders-section { display: none !important; }' : ''}
          ${!selectedSectionsForPrint.changeLog ? '.change-log-section { display: none !important; }' : ''}
          ${!selectedSectionsForPrint.stuckOrders ? '.stuck-orders-section { display: none !important; }' : ''}
          ${!selectedSectionsForPrint.expectationVsTarget ? '.expectation-vs-target-section { display: none !important; }' : ''}
          ${!selectedSectionsForPrint.analysis ? '.analysis-section { display: none !important; }' : ''}
        }
      `}</style>
    </div>
  );
